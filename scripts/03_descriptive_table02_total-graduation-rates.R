...
data_2 <-
    data_raw %>% filter(entra_ES == 1 & 
                 ...
                 mutate(area_conocimiento_cat = as_factor(area_conocimiento_cat) %>% droplevels(),
                        ...
                        teaching_area = ifelse(str_detect(nomb_carrera, "EDUCACION FISICA"), "phys ed", "other ed")
                        )


# denominator: graduates + non-graduates
grads_and_nongrads <-
    data_2 %>% tabyl(q_nse, teaching_area) %>%
    adorn_totals(c("col")) %>%
    select(-q_nse)
grads <-
    data_2 %>% filter(titulado == 1) %>% tabyl(q_nse, teaching_area) %>%
    adorn_totals(c("col")) %>%
    select(-q_nse)

#grads/grads_and_nongrads %>% round_half_up(3)
